<?php
include dirname(__DIR__, 2) . '/config/config.php';
require_once dirname(__DIR__) . '/helpers/auth_guard.php';
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");

$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$user_id = isset($_GET['user_id']) ? (int)$_GET['user_id'] : 0;
$role = $_GET['role'] ?? 'landlord';
$room_id = isset($_GET['room_id']) ? (int)$_GET['room_id'] : 0;
$house_id = isset($_GET['house_id']) ? (int)$_GET['house_id'] : 0;
$managed_house_id = isset($_GET['managed_house_id']) ? (int)$_GET['managed_house_id'] : 0;
$auth = qltro_auth_context($conn);
qltro_apply_auth_context($user_id, $role, $managed_house_id, $auth);

$sql = "SELECT c.*, r.room_name, r.house_id, h.house_name, t.tenant_name, t.phone as tenant_phone 
        FROM contracts c
        JOIN rooms r ON c.room_id = r.id
        JOIN houses h ON r.house_id = h.id
        JOIN tenants t ON c.tenant_id = t.id
        WHERE c.deleted_at IS NULL ";

// 🛡️ PHÂN QUYỀN
if ($role === 'tenant') {
    $sql .= " AND (t.user_id = $user_id OR (t.user_id IS NULL AND t.status = 'active' AND t.deleted_at IS NULL AND t.phone IN (SELECT phone FROM users WHERE id = $user_id))) ";
} else if ($role === 'manager') {
    if ($managed_house_id > 0) {
        $sql .= " AND h.id = $managed_house_id ";
    } else {
        $sql .= " AND h.id = (SELECT managed_house_id FROM users WHERE id = $user_id) ";
    }
} else if ($role !== 'admin') {
    // Landlord
    $sql .= " AND h.user_id = $user_id ";
}

if ($id > 0) {
    $sql .= " AND c.id = $id ";
} else {
    if ($room_id > 0) $sql .= " AND c.room_id = $room_id ";
    if ($house_id > 0) $sql .= " AND h.id = $house_id ";
}

$sql .= " ORDER BY c.id DESC";
$result = $conn->query($sql);
$data = [];

while ($row = $result->fetch_assoc()) {
    $c_id = $row['id'];
    // Lấy thêm danh sách dịch vụ đi kèm hợp đồng nếu là chi tiết
    if ($id > 0) {
        $services = [];
        $s_res = $conn->query("SELECT * FROM contract_services WHERE contract_id = $c_id");
        while ($s_row = $s_res->fetch_assoc()) $services[] = $s_row;
        $row['services'] = $services;

        // Lấy Nhật ký hợp đồng
        $logs = [];
        $log_sql = "SELECT l.*, IFNULL(u.full_name, 'Hệ thống') as user_name, IFNULL(u.role, 'system') as user_role 
                    FROM contract_logs l 
                    LEFT JOIN users u ON l.user_id = u.id 
                    WHERE l.contract_id = $c_id 
                    ORDER BY l.created_at DESC";
        $l_res = $conn->query($log_sql);
        if ($l_res) {
            while($l_row = $l_res->fetch_assoc()) $logs[] = $l_row;
        }
        $row['logs'] = $logs;
    }
    $data[] = $row;
}

if ($id > 0 && count($data) > 0) {
    echo json_encode(["status" => "success", "data" => $data[0]]);
} else {
    echo json_encode(["status" => "success", "data" => $data]);
}
?>
